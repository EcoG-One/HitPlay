...
def _strip_title_parenthetical_terms(title):
    if not title:
        return title
    terms = r"mix|remix|remaster|remastered|anniversary|version|live|single|album|stereo|mono|edit|edition|outtake|explicit|bonus"
...
def get_audio_metadata(file_path):
    ...
    try:
        ...
        wikipedia_date = get_song_release_date(title.strip(), artist.strip())
        if wikipedia_date:
            wikipedia_year = _extract_year(wikipedia_date)
...
def main():
    ...
    serve_directory(script_dir, host, port, state)
